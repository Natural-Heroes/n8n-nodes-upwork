...
async function executeProposalGetMany(itemIndex) {
    const jobPostingId = this.getNodeParameter('jobPostingId', itemIndex);
...
async function executeProposalGet(itemIndex) {
  ...
  `;
    ...
    return data.data?.clientProposal || {};
}
async function executeJobPostCreate(itemIndex) {
    const title = this.getNodeParameter('title', itemIndex);
    const jobDescription = this.getNodeParameter('description', itemIndex);
    const jobType = this.getNodeParameter('jobType', itemIndex);
    ...
    if (jobType === 'FIXED') {
        const budget = this.getNodeParameter('budget', itemIndex, 0);
        if (budget > 0) {
            input.budget = {
                amount: budget,
                currency: 'USD',
        ...
        const hourlyRateRange = this.getNodeParameter('hourlyRateRange', itemIndex, {});
        if (hourlyRateRange.range) {
            const range = hourlyRateRange.range;
            input.hourlyBudget = {
                min: {
                    amount: range.min,
                    currency: 'USD',
    ...
    if (additionalOptions.skills) {
        const skillsString = additionalOptions.skills;
        input.skills = skillsString.split(',').map((s) => s.trim());
    }
  ...
  `;
    ...
    return data.data?.createJobPosting?.jobPosting || {};
}
